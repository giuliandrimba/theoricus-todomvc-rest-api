...
(function() {
  ...
  Database = (function() {
    function Database(name, host, port) {
      ...
      this.mongoUri = process.env.MONGOLAB_URI || process.env.MONGOHQ_URL || "mongodb://localhost/todos_db";
      MongoDatabase.connect(this.mongoUri, function(err, db) {
        _this.db = db;
        if (!err) {
        ...
        if (!err) {
          return collection.findOne({
            ...
            if (!err) {
              ...
              return console.log("Error retrieving todo " + id);
            }
          });
        } else {
          return _this._collection_access_error(callback);
